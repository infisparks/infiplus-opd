...
class _PatientMedicalHistoryPageState extends State<PatientMedicalHistoryPage> {
  ...
  Widget _timingBlock(String label, IconData icon, Map<String, dynamic> med, String k1, String k2) => Container(decoration: BoxDecoration(color: Colors.white, border: Border.all(color: AppColors.border), borderRadius: BorderRadius.circular(12)), child: Column(children: [Padding(padding: const EdgeInsets.all(8), child: Icon(icon, size: 18, color: AppColors.textSecondary)), const Divider(height: 1), _timeToggle("Before", med[k1], (v) => setState(() => med[k1] = v ? "1" : "0")), const Divider(height: 1), _timeToggle("After", med[k2], (v) => setState(() => med[k2] = v ? "1" : "0"))]));
  Widget _timeToggle(String text, dynamic activeVal, Function(bool) onTap) {
    bool active = false;
    if (activeVal is bool) active = activeVal;
    if (activeVal is String) active = activeVal != "0" && activeVal.isNotEmpty;
    return InkWell(
      onTap: () => onTap(!active),
      child: Container(
        height: 34,
        ...
        child: Text(text, style: TextStyle(fontSize: 10, color: active ? Colors.white : AppColors.textSecondary, fontWeight: active ? FontWeight.bold : FontWeight.normal)),
      ),
    );
  }
